...
from model.llama_nofuse import LlamaForCausalLM
...
def main(args):
    ...
    for ii in range(len(model.model.layers)):
        ...
        if model_config.quip_params['outlier_channel_split']:
            layer.self_attn.q_proj.ocs_dupe_inds.copy_(torch.tensor(saved_layer['ocs_dupe_inds']))
        ...
        if model_config.quip_params['outlier_channel_split']:
            layer.self_attn.k_proj.ocs_dupe_inds.copy_(torch.tensor(saved_layer['ocs_dupe_inds']))
        ...
        if model_config.quip_params['outlier_channel_split']:
            layer.self_attn.v_proj.ocs_dupe_inds.copy_(torch.tensor(saved_layer['ocs_dupe_inds']))
        ...
        if model_config.quip_params['outlier_channel_split']:
            layer.self_attn.o_proj.ocs_dupe_inds.copy_(torch.tensor(saved_layer['ocs_dupe_inds']))
