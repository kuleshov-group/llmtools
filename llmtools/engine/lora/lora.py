...
#* ModuLoRA PEFT 0.5.0 Integration *#
import math
import re
import warnings
...
@dataclass
class ModuLoraConfig(PeftConfig):
    """
    ...
    quantization_method: Optional[str] = field(
        default=None,
        metadata={
            "help": "The quantization method for the model, used to integrate QUIP# with Peft."
        },
    )
...
class LoraLayer(BaseTunerLayer):
    ...
    def update_fused_layer(self, adapter_name, fuse_sizes, r, lora_alpha, lora_dropout, init_lora_weights):
        # These are here just to be safe.
        self.r[adapter_name] = r
        self.lora_alpha[adapter_name] = lora_alpha
        self.scaling[adapter_name] = lora_alpha / r 
        for i in range(len(fuse_sizes)):
            adapter_name_fused = f"{self.active_adapter}_{i}"
            self.r[adapter_name_fused] = r
            self.lora_alpha[adapter_name_fused] = lora_alpha
            self.scaling[adapter_name_fused] = lora_alpha / r
            if lora_dropout > 0.0:
                lora_dropout_layer = nn.Dropout(p=lora_dropout)
            else:
                lora_dropout_layer = nn.Identity()
            self.lora_dropout.update(nn.ModuleDict({adapter_name_fused: lora_dropout_layer}))

            self.lora_A.update(nn.ModuleDict({adapter_name_fused: nn.Linear(fuse_sizes[i], r, bias=False)}))
            self.lora_B.update(nn.ModuleDict({adapter_name_fused: nn.Linear(r, fuse_sizes[i], bias=False)}))
            # self.lora_A[adapter_name_fused] = nn.Linear(fuse_sizes[i], r, bias=False)
            # self.lora_B[adapter_name_fused] = nn.Linear(r, fuse_sizes[i], bias=False)
            if init_lora_weights:
                self.reset_lora_parameters(adapter_name_fused)
            self.to(self.weight.device)
...
class ModuLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _check_new_adapter_config(self, config: ModuLoraConfig) -> None:
        """
    ...
    def _create_and_replace(
        ...
        **optionnal_kwargs,
        ...
        #* ModuLoRA Integration: QUIP *#
        #TODO: Figure out how to load this like BNB*#
        # if lora_config.quantization_method == "QUIP":
        if "quip" in str(target).lower(): 
            kwargs["loaded_in_quip"] = True
    ...
    def _replace_module(parent, child_name, new_module, child):
        ...
        #* ModuLoRA Integration: QUIP *#
        if isinstance(child, FusedQuipQuantLinear) and isinstance(new_module, FusedQuipQuantLTLinear):
            old_module = child
            #* Fused Linear Layer *#
            ...
            new_module.weight = old_module.weight # Weight is not stored in QUIP, instead it's stored in Qidxs.
        ...
        elif isinstance(old_module, QuipQuantLinear) and isinstance(new_module, QuipQuantLTLinear):
            old_module = child
            new_module.Qidxs = old_module.Qidxs
            new_module.codebook_id = old_module.codebook_id
            new_module.SU = old_module.SU
    ...
    def _create_new_module(lora_config, adapter_name, target, **kwargs):
        ...
        loaded_in_quip = kwargs.pop("loaded_in_quip", False)
        ...
        #* ModuLoRA Integration *#
        elif loaded_in_quip and isinstance (target, FusedQuipQuantLinear):
            new_module = FusedQuipQuantLTLinear(
                adapter_name,
                target.fuse_dim,
                target.fuse_sizes,
                target.in_features, 
        ...
        elif loaded_in_quip and isinstance(target, QuipQuantLinear):
            new_module = QuipQuantLTLinear(
                adapter_name,         
                target.in_features, 
                target.out_features, 
                target.codesz,
    ...
    #* ModuLoRA Integration Note: Not used currently in the module *#
...
class QuipQuantLTLinear(QuipQuantLinear, LoraLayer):
    ...
    def __init__(
        ...
        # Actual trainable parameters
        if r > 0:
            self.update_layer(adapter_name, r, lora_alpha, lora_dropout, init_lora_weight=init_lora_weights)
            self.active_adapter = adapter_name
...
##* ModuLoRA Quip Implementation *##
class FusedQuipQuantLTLinear(FusedQuipQuantLinear, LoraLayer):
    # Lora implemented in a dense layer
    def __init__(
            self,
            adapter_name,
        ...
        FusedQuipQuantLinear.__init__(self, 
                                fuse_dim=fuse_dim, 
        ...
        if r > 0:
            self.active_adapter = adapter_name
            self.update_fused_layer(adapter_name, fuse_sizes, r, lora_alpha, lora_dropout, init_lora_weights)
